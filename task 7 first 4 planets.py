...
def ORPI():
    ...
    # to calcualte the theta values for the planets
        ...
        radians = [(6*pi / (0.24/ 11.86)),( 6*pi/ (0.62/ 11.86) ),( 6*pi/ (1.00 / 11.86) ),( 6*pi / (1.88/11.86) ),( 6*pi/ 1.00 )]
    ...
    for i in range(4):
        new_x = np.array(x_ani[2]) - np.array(x_ani[i])
        new_y = np.array(y_ani[2]) - np.array( y_ani[i])
